...
New-Item -ItemType Directory -Path "orchestrix\messages" -Force | Out-Null
New-Item -ItemType Directory -Path "orchestrix\reports" -Force | Out-Null
New-Item -ItemType Directory -Path "docs" -Force | Out-Null

# Initialize pm_state.json
if (-not (Test-Path "orchestrix\pm_state.json")) {
...
"@ | Out-File -FilePath "orchestrix\pm_state.json" -Encoding UTF8
} else {
    Write-Host "✓ pm_state.json already exists" -ForegroundColor Gray
}

# Initialize group_status.json
if (-not (Test-Path "orchestrix\group_status.json")) {
...
"@ | Out-File -FilePath "orchestrix\group_status.json" -Encoding UTF8
} else {
    Write-Host "✓ group_status.json already exists" -ForegroundColor Gray
}

# Initialize orchestrator_state.json
if (-not (Test-Path "orchestrix\orchestrator_state.json")) {
...
"@ | Out-File -FilePath "orchestrix\orchestrator_state.json" -Encoding UTF8
} else {
    Write-Host "✓ orchestrator_state.json already exists" -ForegroundColor Gray
}

# Initialize skills_config.json
if (-not (Test-Path "orchestrix\skills_config.json")) {
...
"@ | Out-File -FilePath "orchestrix\skills_config.json" -Encoding UTF8
} else {
...
$MESSAGE_FILES = @(
    ...
    "orchestrix\messages\techlead_to_dev.json"
)
...
if (-not (Test-Path "orchestrix\.gitignore")) {
    Write-Host "📝 Creating orchestrix\.gitignore..." -ForegroundColor Yellow
    @"
...
!.gitignore
"@ | Out-File -FilePath "orchestrix\.gitignore" -Encoding UTF8
} else {
    Write-Host "✓ orchestrix\.gitignore already exists" -ForegroundColor Gray
}
