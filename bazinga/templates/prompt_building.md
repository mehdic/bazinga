...
### Project Context Section (MANDATORY)

**Purpose:** Inject key project context so agents don't need to read files themselves.
...
If file doesn't exist, skip this section (graceful degradation).

**Step 2: Build context summary**

Extract and include these fields in the agent prompt:
...
| Aspect | Value |
|--------|-------|
| **Language** | {primary_language} {version if available} |
| **Framework** | {framework} {version if available} |
| **Architecture** | {architecture_patterns[0..2] as comma-separated} |
...
**Key Utilities:** {common_utilities[0..3].name as comma-separated list}

**Key Directories:**
{FOR each key_directory relevant to task}
- `{path}` - {description}
{ENDFOR}
```

**Step 3: Include research references (if relevant)**

Check `research/` folder for documents matching task keywords:
- If task mentions "OAuth" → check for `research/*oauth*.md`
- If task mentions "database" → check for `research/*database*.md`, `research/*migration*.md`
- If task mentions "API" → check for `research/*api*.md`

Include up to 2 relevant research references:
```markdown
**Relevant Research:**
- `research/oauth2-implementation.md` - OAuth2 provider setup decisions
- `research/api-design-patterns.md` - API design conventions
```

**Token budget:** Keep project context section under 400 tokens. Prioritize:
1. Language/Framework (always include)
2. Conventions (always include)
3. Key utilities (include top 3)
4. Research references (include if directly relevant)
...
**Conventions:**
- File structure: `src/{feature}/{layer}.ts`
- Naming: camelCase for functions, PascalCase for classes

**Key Utilities:** auth_utils, db_utils, validation_utils
...
**Relevant Research:**
- `research/authentication-design.md` - Auth flow decisions
```
