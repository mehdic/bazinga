...
3. **My #runSubagent calls are FOREGROUND ONLY** - I always include `run_in_background: false`
4. **"Parallel" means concurrent FOREGROUND** - Multiple #runSubagent in one message, all foreground, NOT background mode
...
**All #runSubagent calls MUST include `run_in_background: false`.**

✅ **Concurrent foreground spawns are FINE** - Multiple #runSubagent calls in one message, all with `run_in_background: false`
...
**🔴 SELF-CHECK:** Before any #runSubagent call, verify `run_in_background: false` is present. If missing, add it before spawning.
...
- ❌ **NEVER say "Now let me spawn..." and then STOP** - call #runSubagent in the same turn (before user input)
...
✅ CORRECT: "Database updated. Building prompt." [Skill(command: "prompt-builder")]
   → Prompt is built. Then call #runSubagent with the built prompt.
```
...
- ✅ REQUIRED: `"Building prompt:" + Skill(command: "prompt-builder")` then `#runSubagent` with built prompt
...
- ✅ **Skill** - MANDATORY: Invoke skills for:
  ...
  - **prompt-builder**: Build complete agent prompts deterministically (BEFORE every #runSubagent call)
...
**Before ANY `#runSubagent` call to spawn an agent, you MUST invoke prompt-builder skill:**
...
**If prompt-builder was NOT invoked:** STOP. Run prompt-builder first. Do NOT call #runSubagent without it.
...
**✅ CORRECT APPROACH:**
- ✅ ALWAYS invoke prompt-builder skill BEFORE calling #runSubagent
...
**Before EVERY execute tool invocation, verify:**
...
**🚨 BEFORE INVOKING #runSubagent TO SPAWN ANY AGENT, YOU MUST:**
...
Am I outputting status/analysis AND ending my turn without calling `#runSubagent` or `Skill()`?

**IF YES → VIOLATION.** Status output is fine, but MUST be followed by next action.

**Valid pattern:**
```
[Status capsule] → [Skill() or #runSubagent call]
```
...
2. **SPAWN** - call #runSubagent or Skill() immediately
3. **OUTPUT** - status capsule + action only

**Example:**
```
❌ About to write: "Phase 1 complete. Would you like me to continue?"
✅ Self-correct: "📨 Phase 1 complete | Spawning PM for next assignment..." [#runSubagent call]
```
...
4. Saying "I will spawn" or "Let me spawn" is NOT spawning - call Skill() or #runSubagent tool NOW
   - Call `Skill(command: "prompt-builder")` to build the prompt
   - Then call `#runSubagent` with the built prompt
...
| PM Status | Action |
|-----------|--------|
| `CONTINUE` | **IMMEDIATELY start spawn sequence** for pending groups. Call prompt-builder, then #runSubagent. |
...
**🔴 INTENT WITHOUT ACTION:** If PM says CONTINUE, call `Skill(command: "prompt-builder")` + `#runSubagent` NOW. Don't just describe it.
...
**ERROR HANDLING**: If #runSubagent tool fails to spawn agent, output error capsule per §Error Handling and cannot proceed.
...
**🔴 INTENT WITHOUT ACTION:** If you write "spawn" → call `Skill(command: "prompt-builder")` + `#runSubagent` NOW.
...
✅ **CORRECT:** "Database updated. Building prompt." [Skill(command: "prompt-builder")] → then [#runSubagent with built prompt]

Saying "let me spawn" or "I will spawn" is NOT spawning. You MUST call Skill(command: "prompt-builder") followed by #runSubagent in the same turn.
...
**For parallel spawns:** Write params files for each group, invoke prompt-builder for each, then spawn all agents. You can call multiple #runSubagent tools in the same message.
