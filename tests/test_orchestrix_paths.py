#!/usr/bin/env python3
"""
Comprehensive test suite for orchestrix_paths module.
...
def temp_project() -> Generator[Path, None, None]:
    """Create a temporary Orchestrix project structure."""
...
def temp_project_with_db(temp_project: Path) -> Generator[Path, None, None]:
    ...
    db_path = temp_project / "orchestrix" / "orchestrix.db"
    db_path.touch()
    yield temp_project
...
def nested_projects() -> Generator[dict, None, None]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        outer.mkdir()
        (outer / ".claude").mkdir()
        (outer / "orchestrix").mkdir()

        # Inner project
        inner = outer / "packages" / "my_app"
        inner.mkdir(parents=True)
        (inner / ".claude").mkdir()
        (inner / "orchestrix").mkdir()
...
def env_cleanup():
    ...
    old_root = os.environ.get("Orchestrix_ROOT")
    yield
    if old_root is None:
        os.environ.pop("Orchestrix_ROOT", None)
    else:
        os.environ["Orchestrix_ROOT"] = old_root
...
class TestProjectRootValidation:
    ...
    def test_missing_orchestrix_dir(self, temp_project: Path):
        ...
        is_valid, reason = _is_orchestrix_project_root(temp_project)
        assert is_valid is False
        assert "missing" in reason and "orchestrix/" in reason

    def test_both_missing(self):
        """Non-project directory is not valid."""
        with tempfile.TemporaryDirectory() as tmpdir:
            is_valid, reason = _is_orchestrix_project_root(Path(tmpdir))
            assert is_valid is False
...
class TestExplicitOverrides:
    ...
    def test_override_with_invalid_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with pytest.raises(RuntimeError, match="not a valid Orchestrix project root"):
...
class TestEnvironmentVariableOverride:
    ...
    def test_env_var_invalid_no_fallback_raises(self, env_cleanup):
        """Invalid Orchestrix_ROOT raises when no fallback available."""
        with tempfile.TemporaryDirectory() as tmpdir:
            # Set env var to invalid path
            os.environ["Orchestrix_ROOT"] = tmpdir
    ...
    def test_explicit_override_beats_env_var(self, temp_project: Path, env_cleanup):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (other_project / "orchestrix").mkdir()

            os.environ["Orchestrix_ROOT"] = str(other_project)
...
class TestPathDerivation:
    ...
    def test_get_db_path(self, temp_project: Path):
        ...
        assert db_path == temp_project / "orchestrix" / "orchestrix.db"
    ...
    def test_get_artifacts_dir(self, temp_project: Path):
        ...
        assert artifacts == temp_project / "orchestrix" / "artifacts"
    ...
    def test_get_artifacts_dir_with_session(self, temp_project: Path):
        ...
        assert artifacts == temp_project / "orchestrix" / "artifacts" / "test123"
...
class TestRealWorldScenarios:
    ...
    def test_orchestrix_db_script_scenario(self, temp_project_with_db: Path):
        """Simulate orchestrix_db.py script detection."""
        ...
        assert db_path == project / "orchestrix" / "orchestrix.db"
...
class TestErrorHandling:
    ...
    def test_no_project_root_error_message(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                assert ".claude/" in error_msg or "Orchestrix_ROOT" in error_msg
            finally:
                os.chdir(original_cwd)
    ...
    def test_invalid_override_error_message(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            error_msg = str(exc_info.value)
            assert "not a valid Orchestrix project root" in error_msg
