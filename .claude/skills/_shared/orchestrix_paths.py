#!/usr/bin/env python3
"""
Centralized path management for Orchestrix skills.

This module provides robust path resolution that works across:
- Development environment (orchestrix repo)
- Installed environments (client projects after `orchestrix install`)
- Different operating systems (Windows, macOS, Linux)
- Edge cases (monorepos, symlinks, moved directories)

Usage:
    from orchestrix_paths import get_project_root, get_db_path, get_skills_dir
...
def _is_orchestrix_project_root(path: Path) -> Tuple[bool, str]:
    """
    Check if a directory is a valid Orchestrix project root.

    A valid root has BOTH:
    - .claude/ directory (Claude Code configuration)
    - orchestrix/ directory (Orchestrix state/config)

    Returns:
        Tuple of (is_valid, reason_string)
    """
    claude_dir = path / '.claude'
    orchestrix_dir = path / 'orchestrix'

    has_claude = claude_dir.exists() and claude_dir.is_dir()
    has_orchestrix = orchestrix_dir.exists() and orchestrix_dir.is_dir()

    if has_claude and has_orchestrix:
        return True, "found .claude/ and orchestrix/"
    elif has_claude:
        return False, "found .claude/ but missing orchestrix/"
    elif has_orchestrix:
        return False, "found orchestrix/ but missing .claude/"
    else:
        return False, "missing both .claude/ and orchestrix/"
...
def _detect_from_script_location(script_path: Optional[Path] = None) -> Optional[Path]:
    ...
    if script_path is None:
        ...
        # This handles cases where orchestrix_paths is imported
        import inspect
        for frame_info in inspect.stack():
            frame_file = frame_info.filename
            # Skip this module and standard library
            if 'orchestrix_paths' not in frame_file and 'importlib' not in frame_file:
    ...
    while current != current.parent:
        ...
        is_root, _ = _is_orchestrix_project_root(current)
        if is_root:
            return current
...
def _detect_from_env_file() -> Optional[Path]:
    """
    Read project root from .orchestrix/paths.env if it exists.
    ...
    # 1. Check if Orchestrix_ROOT env var is set
    # 2. Try to find .orchestrix/paths.env relative to CWD

    env_root = os.environ.get('Orchestrix_ROOT')
    if env_root:
        env_path = Path(env_root)
        if env_path.exists():
            is_root, _ = _is_orchestrix_project_root(env_path)
            if is_root:
                return env_path
    ...
    try:
        cwd = Path.cwd()
        env_file = cwd / '.orchestrix' / 'paths.env'
        if env_file.exists():
            # Parse the env file for Orchestrix_ROOT
            with open(env_file, 'r') as f:
                for line in f:
                    line = line.strip()
                    if line.startswith('Orchestrix_ROOT='):
                        value = line.split('=', 1)[1].strip('"\'')
                        candidate = Path(value)
                        if candidate.exists():
                            is_root, _ = _is_orchestrix_project_root(candidate)
                            if is_root:
                                return candidate
...
def get_project_root(
    ...
    """
    Get the Orchestrix project root directory.

    Detection order:
    1. Explicit override (--project-root flag)
    2. Orchestrix_ROOT environment variable
    3. .orchestrix/paths.env file (if exists)
    ...
    if override:
        ...
        is_root, reason = _is_orchestrix_project_root(path)
        if not is_root:
            raise RuntimeError(
                f"Override path '{override}' is not a valid Orchestrix project root: {reason}"
            )
    ...
    if _use_cache and _cached_project_root is not None:
        ...
        if _cached_project_root.exists():
            is_root, _ = _is_orchestrix_project_root(_cached_project_root)
            if is_root:
    ...
    if env_root:
        ...
        if path.exists():
            is_root, _ = _is_orchestrix_project_root(path)
            if is_root:
                _cached_project_root = path
                _cached_detection_source = "Orchestrix_ROOT environment variable"
                return path
    ...
    if env_file_root:
        ...
        _cached_detection_source = ".orchestrix/paths.env file"
    ...
    # Failed to detect
    raise RuntimeError(
        "Could not detect Orchestrix project root. "
        "Ensure you are in a directory with both .claude/ and orchestrix/ folders, "
        "or set Orchestrix_ROOT environment variable, "
...
def get_db_path(override: Optional[str] = None, project_root: Optional[Path] = None) -> Path:
    """
    Get the path to the Orchestrix SQLite database.
    ...
    Returns:
        Path to orchestrix.db
    """
    if override:
        return Path(override).resolve()

    root = project_root or get_project_root()
    return root / 'orchestrix' / 'orchestrix.db'
...
def get_artifacts_dir(session_id: Optional[str] = None, project_root: Optional[Path] = None) -> Path:
    ...
    Returns:
        Path to orchestrix/artifacts/ or orchestrix/artifacts/{session_id}/
    """
    root = project_root or get_project_root()
    artifacts = root / 'orchestrix' / 'artifacts'
    if session_id:
...
def get_detection_info() -> dict:
    ...
    try:
        ...
        return {
            ...
            "cwd": str(Path.cwd()),
            "env_var_set": bool(os.environ.get('Orchestrix_ROOT')),
        }
...
def add_shared_to_path():
    ...
    Call this at the top of scripts that need orchestrix_paths:
        import sys
        from pathlib import Path
        sys.path.insert(0, str(Path(__file__).parent.parent / '_shared'))
        from orchestrix_paths import get_project_root

    Or use this helper after initial import:
        from orchestrix_paths import add_shared_to_path
        add_shared_to_path()
    """
