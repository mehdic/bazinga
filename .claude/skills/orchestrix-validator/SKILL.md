---
name: orchestrix-validator
description: Validates Orchestrix completion claims with independent verification. Spawned ONLY when PM sends Orchestrix. Acts as final quality gate - verifies test failures, coverage, evidence, and criteria independently. Returns ACCEPT or REJECT verdict.
version: 1.0.0
allowed-tools: [Bash, Read, Grep, Skill]
---

# Orchestrix Validator Skill

You are the orchestrix-validator skill. When invoked, you independently verify that all success criteria are met before accepting Orchestrix completion signal from the Project Manager.

## When to Invoke This Skill

**Invoke this skill when:**
- Orchestrator receives Orchestrix signal from Project Manager
...
Use the orchestrix-db skill to get success criteria for this session:
...
```
orchestrix-db, please get success criteria for session: [session_id]
```
...
- Default: 60 seconds
- Configurable via `.claude/skills/orchestrix-validator/resources/validator_config.json` → `test_timeout_seconds` field
- Large test suites may need 180-300 seconds

```bash
# Read timeout from config (or use default 60)
TIMEOUT=$(python3 -c "import json; print(json.load(open('.claude/skills/orchestrix-validator/resources/validator_config.json', 'r')).get('test_timeout_seconds', 60))" 2>/dev/null || echo 60)

# Example for Node.js
timeout $TIMEOUT npm test 2>&1 | tee orchestrix/test_output.txt

# Example for Python
timeout $TIMEOUT pytest --tb=short 2>&1 | tee orchestrix/test_output.txt

# Example for Go
timeout $TIMEOUT go test ./... 2>&1 | tee orchestrix/test_output.txt
```
...
IF any test failures exist (count > 0):
  ...
  → PM must fix ALL failures before Orchestrix
```
...
**⚠️ The orchestrator logs this BEFORE invoking you. If no pm_orchestrix event found, REJECT with reason "PM Orchestrix message not found".**

**Step 2: Extract PM's Completion Summary from Orchestrix message**
...
Accept Orchestrix and proceed to shutdown protocol.
```
...
REJECT Orchestrix. Spawn PM with instruction: "375 tests still failing. Continue fixing until failure count = 0."
```
...
→ Action: "Provide recent test output file OR increase test_timeout_seconds in .claude/skills/orchestrix-validator/resources/validator_config.json"
```
...
6. **Timeout protection** - Use configurable timeout (default 60s, see .claude/skills/orchestrix-validator/resources/validator_config.json)
7. **Clear reasoning** - Explain WHY you accepted or rejected

---

**Golden Rule:** "The user expects 100% accuracy when Orchestrix is accepted. Be thorough."
