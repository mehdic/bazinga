...
def init_database(db_path: str) -> None:
    ...
    if current_version < SCHEMA_VERSION:
        ...
        # Handle v18→v19 migration (add platform column to sessions)
        if current_version == 18:
            print("\n--- Migrating v18 → v19 (dual-platform support) ---")
            ...
            if not table_exists:
                # Table will be created later with new schema - skip migration
                print("   ⊘ sessions table will be created with new schema")
            else:
                try:
                    # Add platform column to sessions table
                    cursor.execute("""
                        ALTER TABLE sessions ADD COLUMN platform TEXT DEFAULT 'claude-code'
                    """)
                    print("   ✓ Added sessions.platform (default: 'claude-code')")
                except sqlite3.OperationalError as e:
                    if "duplicate column" in str(e).lower():
                        print("   ⊘ sessions.platform already exists")
                    else:
                        raise

            current_version = 19
            print("✓ Migration to v19 complete (dual-platform support)")

        # Record version upgrade
        cursor.execute("""
            INSERT OR REPLACE INTO schema_version (version, description)
            VALUES (?, ?)
        """, (SCHEMA_VERSION, f"Schema v{SCHEMA_VERSION}: Dual-platform support (claude-code + copilot)"))
        conn.commit()
    ...
    # Extended in v19 to support platform tracking (claude-code, copilot)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS sessions (
            ...
            platform TEXT DEFAULT 'claude-code' CHECK(platform IN ('claude-code', 'copilot')),
            metadata TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    """)
