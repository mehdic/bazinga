...
class BazingaDB:
    ...
    def create_task_group(self, group_id: str, session_id: str, name: str,
        ...
        already exists, only name/status/assigned_to/specializations/item_count/component_path/initial_tier
        ...
        Args:
            ...
            initial_tier: Initial implementation tier ('Developer' or 'Senior Software Engineer')
        ...
        try:
            ...
            if initial_tier is not None and initial_tier not in valid_tiers:
                return {
                    "success": False,
                    "error": f"initial_tier must be one of: {', '.join(valid_tiers)}"
                }
...
TASK GROUP OPERATIONS:
  create-task-group <group_id> <session> <name> [status] [assigned_to] [--specializations JSON] [--component-path PATH] [--initial_tier TIER]
                                              Create task group (TIER: 'Developer' or 'Senior Software Engineer')
  update-task-group <group_id> <session> [--status X] [--assigned_to Y] [--specializations JSON] [--component-path PATH] [--initial_tier TIER]
                                              Update task group (specializations as JSON array, component_path for version context)
  get-task-groups <session> [status]          Get task groups (includes specializations, component_path, initial_tier)
        ...
        if cmd == 'create-session':
            ...
            while i < len(cmd_args):
                ...
                if arg_normalized == '--specializations' and i + 1 < len(cmd_args):
                    ...
                    valid_tiers = ('Developer', 'Senior Software Engineer')
                    if initial_tier not in valid_tiers:
                        print(json.dumps({"success": False, "error": f"--initial_tier must be one of: {', '.join(valid_tiers)}"}, indent=2), file=sys.stderr)
                        sys.exit(1)
                    i += 2  # Skip flag and value
                else:
                    positional_args.append(cmd_args[i])
            ...
            # v16: Added initial_tier for PM to specify starting agent tier
