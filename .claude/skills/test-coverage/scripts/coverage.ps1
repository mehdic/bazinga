...
switch ($LANG) {
    "python" {
        ...
        if (-not $?) {
            ...
            '{"totals":{"percent_covered":0},"files":{}}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
        }

        # pytest-cov outputs to coverage.json by default
        if (Test-Path "coverage.json") {
            Move-Item "coverage.json" "orchestrix\coverage_report_raw.json" -Force
        } elseif (-not (Test-Path "orchestrix\coverage_report_raw.json")) {
            '{"totals":{"percent_covered":0},"files":{}}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
        }
    }
    ...
    "javascript" {
        ...
        if (-not (Test-Path "node_modules\.bin\jest") -and -not (Test-Path "node_modules\.bin\jest.cmd")) {
            ...
            '{"coverageMap":{}}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
        } else {
            Write-Host "  Running jest with coverage..." -ForegroundColor Gray
            npm test -- --coverage --json --outputFile=orchestrix\jest-results.json 2>$null
            if (-not $?) {
                Write-Host "⚠️  Tests failed or no tests found" -ForegroundColor Yellow
                '{"coverageMap":{}}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
            }
            ...
            if (Test-Path "coverage\coverage-final.json") {
                Copy-Item "coverage\coverage-final.json" "orchestrix\coverage_report_raw.json"
            } elseif (-not (Test-Path "orchestrix\coverage_report_raw.json")) {
                '{"coverageMap":{}}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
            }
        }
    ...
    "go" {
        ...
        go test -coverprofile=orchestrix\coverage.out .\... 2>$null
        if (-not $?) {
            Write-Host "⚠️  Tests failed or no tests found" -ForegroundColor Yellow
            '{"coverage":0}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
        }

        if (Test-Path "orchestrix\coverage.out") {
            # Parse go coverage output
            $coverageOutput = go tool cover -func=orchestrix\coverage.out | Select-String "total"
            if ($coverageOutput) {
                $COVERAGE = ($coverageOutput -replace '.*\s(\d+\.\d+)%.*','$1')
                "{`"coverage`":$COVERAGE}" | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
            } else {
                '{"coverage":0}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
            }
        } elseif (-not (Test-Path "orchestrix\coverage_report_raw.json")) {
        ...
        if (Test-Path "pom.xml") {
            if (Test-CommandExists "mvn") {
                ...
                if (Test-Path "target\site\jacoco\jacoco.xml") {
                    ...
                    if ($total -gt 0) {
                        ...
                        "{`"coverage`":$coverage,`"source`":`"target/site/jacoco/jacoco.xml`"}" | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
                    } else {
                        '{"coverage":0}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
                    }
                } elseif (-not (Test-Path "orchestrix\coverage_report_raw.json")) {
                ...
                '{"error":"Maven not found"}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
            }
            ...
            if ($GRADLE_CMD) {
                ...
                if (Test-Path "build\reports\jacoco\test\jacocoTestReport.xml") {
                    ...
                    if ($total -gt 0) {
                        $coverage = [math]::Round(($lineCovered / $total) * 100, 2)
                        "{`"coverage`":$coverage,`"source`":`"build/reports/jacoco/test/jacocoTestReport.xml`"}" | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
                    } else {
                        '{"coverage":0}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
                    }
                } elseif (-not (Test-Path "orchestrix\coverage_report_raw.json")) {
                ...
                '{"error":"Gradle not found"}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
            }
        } else {
            Write-Host "❌ No Maven or Gradle build file found" -ForegroundColor Red
            '{"error":"No build file"}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
        }
    }

    default {
        Write-Host "❌ Unknown language. Cannot run coverage analysis." -ForegroundColor Red
        '{"error":"Unknown language"}' | Out-File -FilePath "orchestrix\coverage_report_raw.json" -Encoding UTF8
    }
}
...
$rawData = Get-Content "orchestrix\coverage_report_raw.json" -Raw | ConvertFrom-Json
...
$finalReport | ConvertTo-Json -Depth 10 | Out-File -FilePath "orchestrix\coverage_report.json" -Encoding UTF8

# Clean up
Remove-Item "orchestrix\coverage_report_raw.json" -ErrorAction SilentlyContinue
Remove-Item "orchestrix\jest-results.json" -ErrorAction SilentlyContinue

Write-Host "✅ Coverage analysis complete" -ForegroundColor Green
Write-Host "📁 Results saved to: orchestrix\coverage_report.json" -ForegroundColor Cyan

# Display summary
$reportData = Get-Content "orchestrix\coverage_report.json" -Raw | ConvertFrom-Json
