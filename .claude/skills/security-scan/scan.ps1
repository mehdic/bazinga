                ...
                if (Test-Path "pom.xml") {
                    ...
                    if (Test-CommandExists "mvn") {
                        ...
                        if (Test-Path "target\spotbugsXml.xml") {
                            '{"tool":"spotbugs","source":"target/spotbugsXml.xml"}' | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
                        } else {
                            '{"issues":[]}' | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
                        }
                    } else {
                    ...
                    if ($GRADLE_CMD) {
                        ...
                        if (Test-Path "build\reports\spotbugs\main.xml") {
                            '{"tool":"spotbugs","source":"build/reports/spotbugs/main.xml"}' | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
                        } else {
                            '{"issues":[]}' | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
                        }
                    } else {
        ...
        switch ($LANG) {
            "python" {
                ...
                if ($SCAN_STATUS -ne "error") {
                    ...
                    $banditData = Get-Content "coordination\bandit_full.json" -Raw | ConvertFrom-Json
                    $semgrepData = Get-Content "coordination\semgrep.json" -Raw | ConvertFrom-Json
                    @{
                        bandit = $banditData
                        semgrep = $semgrepData
                    } | ConvertTo-Json -Depth 10 | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
                }
            }
            ...
            "javascript" {
                ...
                if ($?) {
                    ...
                    npx eslint . --plugin security --format json > coordination\eslint_security.json 2>$null
                    if (-not $?) {
                        $SCAN_STATUS = "partial"
                        $SCAN_ERROR = if ($SCAN_ERROR) { "$SCAN_ERROR; eslint-security scan failed" } else { "eslint-security scan failed" }
                        '[]' | Out-File -FilePath "coordination\eslint_security.json" -Encoding UTF8
                    }

                    # Combine results
                    $npmData = Get-Content "coordination\npm_audit.json" -Raw | ConvertFrom-Json
                    $eslintData = Get-Content "coordination\eslint_security.json" -Raw | ConvertFrom-Json
                    @{
                        npm_audit = $npmData
                        eslint = $eslintData
                    } | ConvertTo-Json -Depth 10 | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
            ...
            "java" {
                ...
                if (Test-CommandExists "semgrep") {
                    ...
                    semgrep --config=auto --json -o coordination\semgrep_java.json 2>$null
                    if (-not $?) {
                        $SCAN_STATUS = "partial"
                        $SCAN_ERROR = if ($SCAN_ERROR) { "$SCAN_ERROR; Semgrep scan failed" } else { "Semgrep scan failed" }
                        '{"results":[]}' | Out-File -FilePath "coordination\semgrep_java.json" -Encoding UTF8
                    }
                }

                # Consolidate Java results
                '{"tool":"spotbugs+owasp+semgrep","status":"see_build_reports"}' | Out-File -FilePath "coordination\security_scan_raw.json" -Encoding UTF8
            }
...
$rawData = Get-Content "coordination\security_scan_raw.json" -Raw | ConvertFrom-Json
...
$finalReport | ConvertTo-Json -Depth 10 | Out-File -FilePath "coordination\security_scan.json" -Encoding UTF8
...
Remove-Item "coordination\semgrep_java.json" -ErrorAction SilentlyContinue
