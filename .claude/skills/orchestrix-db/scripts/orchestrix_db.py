#!/usr/bin/env python3
"""
Orchestrix Database Client - Simple command interface for database operations.
...
Path Resolution:
    ...
    - --project-root DIR Explicit project root (db at DIR/orchestrix/orchestrix.db)
    - Orchestrix_ROOT env   Environment variable override
...
# Path: .claude/skills/orchestrix-db/scripts/orchestrix_db.py
#   -> .claude/skills/orchestrix-db/scripts/  (parent)
#   -> .claude/skills/orchestrix-db/          (parent.parent)
#   -> .claude/skills/                      (parent.parent.parent)
#   -> .claude/skills/_shared/              (where orchestrix_paths.py lives)
...
try:
    from orchestrix_paths import get_project_root, get_db_path, get_detection_info
...
class OrchestrixDB:
    """Database client for Orchestrix orchestration."""
        ...
        try:
            ...
            if project_root is None:
                if _HAS_Orchestrix_PATHS:
                    ...
                    # Fallback: assume db_path is at PROJECT_ROOT/orchestrix/orchestrix.db
                    project_root = Path(self.db_path).parent.parent

            # Define allowed base directory
            allowed_base = (project_root / "orchestrix" / "templates" / "specializations").resolve()
    ...
    def log_interaction(self, session_id: str, agent_type: str, content: str,
        ...
        # Per schema v2 migration, Orchestrix is designed to be extensible.
    ...
    def save_event(self, session_id: str, event_subtype: str, payload: str,
        ...
        Used for: pm_orchestrix, scope_change, validator_verdict events.

        Args:
            session_id: The session ID
            event_subtype: Type of event (pm_orchestrix, scope_change, validator_verdict)
    ...
    def get_events(self, session_id: str, event_subtype: Optional[str] = None,
        ...
        Args:
            session_id: The session ID
            event_subtype: Optional filter by event type (pm_orchestrix, scope_change, etc.)
    ...
    def save_context_package(self, session_id: str, group_id: str, package_type: str,
        ...
        artifacts_root = Path("orchestrix/artifacts") / session_id
        ...
        normalized_path = f"orchestrix/artifacts/{session_id}/{str(rel_path).replace(os.sep, '/')}"
...
def print_help():
    ...
    help_text = """
Orchestrix Database Client - Available Commands:
...
PATH RESOLUTION:
  ...
  --db PATH           Explicit database path
  --project-root DIR  Project root (db at DIR/orchestrix/orchestrix.db)
  Orchestrix_ROOT env    Environment variable

Examples:
  orchestrix_db.py list-sessions 5                                    # Auto-detect path
  orchestrix_db.py --db orchestrix.db list-sessions 5                    # Explicit path
  orchestrix_db.py --project-root /path/to/project list-sessions 5    # Project root
  orchestrix_db.py detect-paths                                       # Show detected paths
  orchestrix_db.py query "SELECT * FROM sessions LIMIT 3"
  orchestrix_db.py get-task-groups session123
"""
    print(help_text)
...
def _resolve_db_path(args) -> str:
    ...
    # 2. Try auto-detection via orchestrix_paths module
    if _HAS_Orchestrix_PATHS:
        try:
            db_path = get_db_path(
                override=args.db,
            ...
            print("Hint: Use --db to specify path explicitly, or ensure you're in a Orchestrix project.", file=sys.stderr)
            sys.exit(1)
    else:
        ...
        for _ in range(10):  # Max 10 levels up
            candidate = current / 'orchestrix' / 'orchestrix.db'
            if (current / '.claude').exists() and (current / 'orchestrix').exists():
...
def main():
    parser = argparse.ArgumentParser(
        description='Orchestrix Database Client',
    ...
    if args.command == 'detect-paths':
        if _HAS_Orchestrix_PATHS:
            ...
            print(json.dumps({"error": "orchestrix_paths module not available"}, indent=2))
        sys.exit(0)

    # Resolve database path
    db_path = _resolve_db_path(args)
    db = OrchestrixDB(db_path, quiet=args.quiet)
    ...
    try:
        ...
        # ==================== STRATEGIES COMMANDS ====================
            ...
            if len(cmd_args) < 3:
                ...
                print("  event_subtype: pm_orchestrix, scope_change, validator_verdict", file=sys.stderr)
            ...
            if len(cmd_args) < 1:
                ...
                print("    get-events sess_123 pm_orchestrix         # filter by subtype", file=sys.stderr)
                print("    get-events sess_123 pm_orchestrix 1       # with limit", file=sys.stderr)
                sys.exit(1)
            session_id = cmd_args[0]
            event_subtype = None
            limit = 50
