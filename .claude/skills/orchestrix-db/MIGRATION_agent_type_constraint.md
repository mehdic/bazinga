...
**Reason:** Orchestrix is designed to be extensible. New agents can be added without schema changes.
...
cp orchestrix/orchestrix.db orchestrix/orchestrix.db.backup

# Remove old database
rm orchestrix/orchestrix.db

# Reinitialize with new schema
python .claude/skills/orchestrix-db/scripts/init_db.py orchestrix/orchestrix.db
```
...
sqlite3 orchestrix/orchestrix.db <<EOF
.headers on
.mode csv
.output orchestrix/orchestration_logs_backup.csv
...
sqlite3 orchestrix/orchestrix.db <<EOF
.mode csv
.import orchestrix/orchestration_logs_backup.csv orchestration_logs_temp
...
# 5. Verify data
sqlite3 orchestrix/orchestrix.db "SELECT COUNT(*) FROM orchestration_logs;"

# 6. Clean up
rm orchestrix/orchestration_logs_backup.csv
```
...
```powershell
# Backup existing database (optional)
Copy-Item orchestrix/orchestrix.db orchestrix/orchestrix.db.backup -ErrorAction SilentlyContinue

# Remove old database
Remove-Item orchestrix/orchestrix.db -ErrorAction SilentlyContinue
...
```powershell
# 1. Backup database
Copy-Item orchestrix/orchestrix.db orchestrix/orchestrix.db.backup

# 2. Use Python to run SQL commands
$dbPath = "orchestrix/orchestrix.db"
...
```bash
sqlite3 orchestrix/orchestrix.db "PRAGMA table_info(orchestration_logs);"
```
...
- Check backup: `orchestrix/orchestrix.db.backup`
- Verify export: `orchestrix/orchestration_logs_backup.csv`
- Restore: `cp orchestrix/orchestrix.db.backup orchestrix/orchestrix.db`

---

## Related Files Changed

- `.claude/skills/orchestrix-db/scripts/init_db.py` (line 47)
- `.claude/skills/orchestrix-db/references/schema.md` (line 75)
