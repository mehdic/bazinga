---
name: orchestrix-db
description: Database operations for Orchestrix orchestration system. This skill should be used when agents need to save or retrieve orchestration state, logs, task groups, token usage, or skill outputs. Replaces file-based storage with concurrent-safe SQLite database. Use instead of writing to orchestrix/*.json files or docs/orchestration-log.md.
version: 1.0.0
...
You are the orchestrix-db skill. When invoked, you handle database operations for the Orchestrix orchestration system, providing concurrent-safe storage for sessions, logs, state, task groups, tokens, and skill outputs.
...
- Any agent mentions "save to database", "query database", or "orchestrix-db"
- PM needs to save/retrieve multi-phase development plans
- PM needs to save success criteria at planning phase
- PM needs to update success criteria status before Orchestrix
- Orchestrator needs to query success criteria for Orchestrix validation
- Replacing file writes to `orchestrix/*.json` or `docs/orchestration-log.md`
...
2. `--project-root DIR` flag (db at DIR/orchestrix/orchestrix.db)
3. `Orchestrix_ROOT` environment variable
4. Auto-detect from script location (walks up to find .claude/ and orchestrix/)
...
**Script path:** `.claude/skills/orchestrix-db/scripts/orchestrix_db.py` (auto-detects database)

### Session Management

**List recent sessions:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet list-sessions [limit]
```
Returns JSON array of recent sessions (default 10, ordered by created_at DESC).

**Create new session:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet create-session \
  "<session_id>" \
  "<mode>" \
  "<requirements>" \
...
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet log-interaction \
  "<session_id>" "<agent_type>" "<content>" [iteration] [agent_id]
```

**Save state (PM or orchestrator):**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet save-state \
  "<session_id>" "<state_type>" '<json_data>'
```

**Get latest state:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet get-state \
  "<session_id>" "<state_type>"
```

**Create task group:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet create-task-group \
  "<group_id>" "<session_id>" "<name>" [status] [assigned_to] \
  [--specializations '<json_array>'] [--item_count N]
```

Parameters:
- `specializations`: JSON array of specialization file paths (e.g., `'["orchestrix/templates/specializations/01-languages/typescript.md"]'`)
- `item_count`: Number of discrete tasks/items in this group (used for progress tracking)

**Update task group:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet update-task-group \
  "<group_id>" "<session_id>" [--status "<status>"] [--assigned_to "<agent_id>"] [--specializations '<json_array>']
```

**Get task groups:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet get-task-groups \
  "<session_id>" [status]
```

**Update session status:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet update-session-status \
  "<session_id>" "<status>"
```
...
**Dashboard snapshot:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet dashboard-snapshot \
  "<session_id>"
```

**Save development plan:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet save-development-plan \
...
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet get-development-plan \
  "<session_id>"
```

**Update plan progress:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet update-plan-progress \
  "<session_id>" <phase_number> "<status>"
```

### Success Criteria (Orchestrix Validation)

**Save success criteria (PM during planning):**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet save-success-criteria \
  "<session_id>" '[{"criterion":"All tests passing","status":"pending"}]'
```

**Get success criteria (Orchestrator for Orchestrix validation):**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet get-success-criteria \
  "<session_id>"
```

**Update success criterion (PM before Orchestrix):**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet update-success-criterion \
...
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet save-context-package \
...
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet mark-context-consumed \
  "<package_id>" "<agent_type>" [iteration]
```

**Update task group context references:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet update-context-references \
...
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet get-reasoning \
  "<session_id>" [--group_id X] [--agent_type Y] [--phase Z] [--limit N]
```

**Get reasoning timeline:**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet reasoning-timeline \
  "<session_id>" [--group_id X] [--format json|markdown]
```

**Check mandatory phases (for workflow validation):**
```bash
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet check-mandatory-phases \
  "<session_id>" "<group_id>" "<agent_type>"
```
Returns exit code 1 if mandatory phases (understanding, completion) are missing.

**Full command reference:** See `scripts/orchestrix_db.py --help` for all available operations.
...
[Raw output from orchestrix_db.py script]
```
...
- Session 1: orchestrix_123 (most recent)
- Session 2: orchestrix_456

Recommendation: Resume session orchestrix_123
```
...
Command:
python3 /path/to/orchestrix_db.py --db /path/to/orchestrix.db create-session ...
...
Session ID: orchestrix_20250113_143530
Mode: simple
...
Expected output:
```
✓ Database auto-initialized at orchestrix/orchestrix.db
✓ Session created: orchestrix_20250113_143530
...
Expected output:
```
❌ Database not found at: orchestrix/orchestrix.db
...
Session ID: orchestrix_20250203_143530
Group ID: group_a
Type: research
File: orchestrix/artifacts/orchestrix_20250203_143530/context/research-group_a-hin.md
...
{
  "package_id": 1,
  "file_path": "orchestrix/artifacts/orchestrix_20250203_143530/context/research-group_a-hin.md",
  "consumers_created": 2
}
```
...
Request from orchestrator:
```
orchestrix-db, get context packages for developer spawn:
...
```
orchestrix-db, save my reasoning:
...
```json
{
  "success": true,
  "log_id": 42,
  "session_id": "orchestrix_20250203_143530",
  "group_id": "group_a",
...
```
orchestrix-db, get reasoning timeline:
...
- The script (`orchestrix_db.py`) handles all SQL operations and concurrency
...
**Script:** `python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet <command>`

**Most common operations:**

```bash
# Log interaction
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet log-interaction "$SID" "pm" "$CONTENT" 1

# Save PM state
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet save-state "$SID" "pm" '{"iteration":1}'

# Update task group
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet update-task-group "group_a" "$SID" --status "completed"

# Dashboard snapshot
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py --quiet dashboard-snapshot "$SID"

# Detect paths (debugging)
python3 .claude/skills/orchestrix-db/scripts/orchestrix_db.py detect-paths
```
