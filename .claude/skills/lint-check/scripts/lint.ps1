...
switch ($LANG) {
    "python" {
        ...
        if (Test-CommandExists "ruff") {
            ...
            ruff check . --output-format=json > orchestrix\lint_results_raw.json 2>$null
            if (-not $?) {
                '[]' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
        } elseif (Test-CommandExists "pylint") {
            $TOOL = "pylint"
            Write-Host "  Running pylint..." -ForegroundColor Gray
            pylint --output-format=json **/*.py > orchestrix\lint_results_raw.json 2>$null
            if (-not $?) {
                '[]' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
        } else {
    ...
    "javascript" {
        ...
        if ((Test-Path "node_modules\.bin\eslint") -or (Test-Path "node_modules\.bin\eslint.cmd") -or (Test-CommandExists "eslint")) {
            ...
            npx eslint . --format json > orchestrix\lint_results_raw.json 2>$null
            if (-not $?) {
                '[]' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
        } else {
    ...
    "go" {
        ...
        if (Test-CommandExists "golangci-lint") {
            ...
            golangci-lint run --out-format json > orchestrix\lint_results_raw.json 2>$null
            if (-not $?) {
                '{"Issues":[]}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
        } else {
    ...
    "ruby" {
        ...
        if (Test-CommandExists "rubocop") {
            ...
            rubocop --format json > orchestrix\lint_results_raw.json 2>$null
            if (-not $?) {
                '{"files":[]}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
        } else {
        ...
        if (Test-Path "pom.xml") {
            if (Test-CommandExists "mvn") {
                ...
                if ((Test-Path "target\checkstyle-result.xml") -or (Test-Path "target\pmd.xml")) {
                    '{"tool":"checkstyle+pmd","checkstyle":"target/checkstyle-result.xml","pmd":"target/pmd.xml"}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
                } else {
                    '{"issues":[]}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
                }
            } else {
                ...
                $TOOL = "none"
                '{"error":"Maven not found"}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
            ...
            if ($GRADLE_CMD) {
                ...
                if ((Test-Path "build\reports\checkstyle\main.xml") -or (Test-Path "build\reports\pmd\main.xml")) {
                    '{"tool":"checkstyle+pmd","checkstyle":"build/reports/checkstyle/main.xml","pmd":"build/reports/pmd/main.xml"}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
                } else {
                    '{"issues":[]}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
                }
            } else {
                ...
                $TOOL = "none"
                '{"error":"Gradle not found"}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
            }
        } else {
            Write-Host "❌ No Maven or Gradle build file found" -ForegroundColor Red
            $TOOL = "none"
            '{"error":"No build file"}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
        }
    }

    default {
        Write-Host "❌ Unknown language. Cannot run linting." -ForegroundColor Red
        '{"error":"Unknown language"}' | Out-File -FilePath "orchestrix\lint_results_raw.json" -Encoding UTF8
    }
}
...
# Read raw results
$rawResults = Get-Content "orchestrix\lint_results_raw.json" -Raw
...
"@ | Out-File -FilePath "orchestrix\lint_results.json" -Encoding UTF8

# Clean up
Remove-Item "orchestrix\lint_results_raw.json" -ErrorAction SilentlyContinue

Write-Host "✅ Linting complete" -ForegroundColor Green
Write-Host "📁 Results saved to: orchestrix\lint_results.json" -ForegroundColor Cyan
