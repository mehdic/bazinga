---
description: Execute spec-kit tasks using Orchestrix multi-agent orchestration with full integration
---

# Orchestrix + Spec-Kit Integration Command

This command bridges spec-kit's planning phase with Orchestrix's execution phase, creating a seamless spec-driven development workflow.
...
- **Detailed report** saved to `orchestrix/artifacts/{SESSION_ID}/completion_report.md` with comprehensive analysis
...
3. Uses Skills configured in orchestrix/skills_config.json
...
**Integration Flow**:
```
spec-kit (planning) → /orchestrate-from-spec → Orchestrix (execution)
```

This command:
1. ✅ Loads spec-kit artifacts (spec.md, tasks.md, plan.md)
2. ✅ Validates they exist and are complete
3. ✅ Spawns Orchestrix orchestrator with spec-kit context
...
**Next**: Spawning Orchestrix orchestrator to execute tasks...
...
## Phase 2: Spawn Orchestrix Orchestrator with Spec-Kit Context
...
✅ **Skills Mode**: Advanced Skills enabled per orchestrix/skills_config.json
...
5. **Create Orchestrix Groups from Spec-Kit Tasks** (Step 5):
...
**Skills Available** (if marked as 'mandatory' in orchestrix/skills_config.json):
...
3. **Review automated Skill results**:
   - Security scan findings from orchestrix/security_scan.json
   - Coverage report from orchestrix/coverage_report.json
   - Lint results from orchestrix/lint_results.json
...
5. **Complete**: PM sends Orchestrix when all tasks in tasks.md are checked
...
- 🆕 **Revision tracking**: orchestrix/group_status.json tracks per-group revision counts
...
Task(
  ...
  description: "Orchestrix orchestration executing spec-kit tasks",
...
While orchestration runs, you can monitor progress by invoking the orchestrix-db skill:

**To check orchestration logs:**

Request to orchestrix-db skill:
```
orchestrix-db, please stream recent logs for session {session_id}.
...
Request to orchestrix-db skill:
```
orchestrix-db, please get the PM state for session {session_id}.
...
**Note:** All orchestration data is stored in orchestrix/orchestrix.db. Use the orchestrix-db skill to query it instead of direct bash commands.

### Step 3.2: Handle Completion

When orchestrator returns with Orchestrix from PM:
...
✅ **Parallel Execution**: Orchestrix's adaptive parallelism
...
# Step 5: Execute with Orchestrix (THIS COMMAND)
...
- This command requires both Orchestrix and spec-kit to be installed
- Spec-kit artifacts must be created before running this command
- tasks.md serves as shared state between spec-kit and Orchestrix
- All standard Orchestrix features work (parallel mode, role drift prevention, etc.)
...
**Created**: 2025-11-07
**Version**: 1.0.0
**Integration**: Orchestrix v1.0 + Spec-Kit compatible
